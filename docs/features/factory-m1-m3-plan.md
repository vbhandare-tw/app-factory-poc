...
- **The base branch is never verified as green *before* the feature merges into it.** The close gates the feature branch; nothing gates the base branch. So a base branch that is already red takes the feature's merge anyway, and the requirements §16 acceptance item "a deliberately red base branch blocks merge" is **not** satisfied by Phase 11. It is listed under Phase 12's acceptance list; whoever runs that phase should expect to add a pre-merge base gate rather than to discover one.
