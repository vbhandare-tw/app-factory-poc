...
| 9 docs + acceptance | `4034fe1` | docs only (suite unchanged: 2223 / 12 / 100, 3 pin) | lint ok · typecheck ok · ci+scaffold 24/24 | Orchestrator read ADR-005 and fixed 2 sentences (demo file locations; `claim_released` is logged but the release writes nothing on a pause). Browser acceptance (Chrome, fresh demo): demo blocks Add feature; Send back disabled until a note is typed, then enabled with a single focused textarea kept; no "Sending…" while the merge confirm is open; Cancel restores the buttons. The full merge-to-main path through the page was proven in the Phase 8 walk. Field Guide republished (dashboard, demo, cheat sheet) | 400 px layout still not checked in a real browser. The demo refusal text shows literal backticks. The `/session-summary`, PR and paid real-agent run are left for the human. |
| code-review-fix (2026-09-25) | _this commit_ | 2230 / 12 / 100 (3 pin) | pin only · ok · ok · ok | Whole-feature review (high): 10 findings. Fixed 3: a malformed known-type event no longer 500s the feed or drops later lines from the tail (summariseEvent falls back, per-line isolation); the EventLog is closed if workspace or runner construction throws in `startOrchestrator`; `--` after the revision range in `logRange`/`diffNumstat`. A 4th fix (re-read config.yml on Start) was reverted after round 2 showed it only partly applied, so a page merge could use a stale `base_branch` (human decision: one config read at launch, used everywhere; the refusal panel now says to restart the dashboard after editing config.yml). Rejected: heartbeat age for mode (plan A8, a live PID is authoritative), `claim_released` after a no-op release (accepted in the 8b review), the double vault scan and whole-file reads (fine at one-feature scale), `hosted` briefly shown during a losing start (harmless), the slug wrapper (the mirror test uses it) | Follow-ups: one shared config source so edits apply without a restart; `--end-of-options` before revision ranges (a ref starting with `-`); remove the abort listener on successful shutdown; `summariseEvent` should log swallowed formatter errors; the unused `ReadContext.demo` override; scan-once `/api/state`; tail reads for large logs. |
