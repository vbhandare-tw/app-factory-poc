...
| Merge conflict | `needs_human`, `pause_reason: merge_conflict` (no agent retry in M1–M3). **`attempts` unchanged** — added during Phase 10, making explicit what this row already implied by omitting the `attempts += 1` every other row spells out. Neither merge failure has a retry, so a count would be a number nobody acts on, and charging it would report a ticket whose agent ran once cleanly as having failed three times because two branches disagreed about a line. |
| Red gate **on the feature branch, after the merge** | *(Added during Phase 10 — this row did not exist.)* `needs_human`, `pause_reason: escalation`, `attempts` unchanged. Deliberately **not** `merge_conflict`: there are no conflict markers to find, and a human sent looking for them wastes the trip. The ticket passed its own gates, so this is the *combination* failing, which no re-run of anything can fix. The merge commit is reverted — the feature branch goes back to the SHA it had before the merge — because `git merge --abort` cannot undo a merge that has already been committed, and a feature branch left carrying a merge that fails its own gates makes every later ticket merge onto a broken branch and look like the culprit. Nothing is lost: the ticket branch still holds every commit, and the merge commit is reproducible by definition. |
| Red gate **on the feature branch, before final acceptance** | *(Added during Phase 11 — this row did not exist.)* Feature to `needs_human`, `pause_reason: escalation`, `resume_to: awaiting_feature_close`, `reject_to: in_development`, `attempts` unchanged. **The `final_acceptance` checkpoint is never offered.** The checkpoint's whole value is that a human approves something already verified; offering a red branch asks them to authorise a base-branch merge on evidence that says not to, and their answer would then be the only thing standing between a red branch and the base branch. Reachable even though Phase 10 guarantees the branch is green after every *merge*, because that guarantee says nothing about what lands on the branch afterwards — a hand-applied hotfix, a colleague, a rebase gone wrong. |
| Base-branch merge conflict at final acceptance | *(Added during Phase 11.)* Feature stays at `needs_human`, `pause_reason: merge_conflict`, `resume_to: done` so that fixing the branch and approving again comes straight back to the close, `attempts` unchanged. `git merge --abort` has already run, so the base branch is exactly where it was and nothing is tagged. **Never retried automatically and no agent resolves it** (§10, ADR-004). `factory approve` exits non-zero and the reason is written into the note, because an operator whose approval failed reads `NEEDS_HUMAN.md` next, not their terminal. |
| The feature branch **moved** between the checkpoint and the approval | *(Added during Phase 11's review.)* `factory approve` refuses, `pause_reason: escalation`, `resume_to: awaiting_feature_close`, nothing merged and nothing tagged. The gates run before the checkpoint and the approval arrives whenever a person gets to it, so `verified_sha` on the feature note records the commit the gates actually passed and the approval re-checks it against the branch tip. A missing `verified_sha` refuses too — absent evidence is not evidence, as with `mergeVerified`. This is **not** a re-run of the gates: a commit added after the checkpoint may be green and is still one no approver saw, and the property being kept is that the thing a human approved is the thing that lands. Approving again sends the feature back to `awaiting_feature_close` to be verified afresh, so the refusal is not a dead end. |
| The base merge landed and the **tag** failed | *(Added during Phase 11.)* Feature stays at `needs_human`, `pause_reason: escalation`, `resume_to: done`. **Nothing is rewound** — see §10 for the argument. The feature must not reach `done` because the transition guard has no tag to be given, so the vault will not record a delivery without the marker of what was delivered. Approving again is safe and is the documented fix: `git merge --no-ff` of an already-merged branch reports success without a new commit, so the second attempt goes straight to the tag. |
...
*(Added during Phase 11 — four things the "Merge direction" bullet above leaves open, each decided by the code that had to answer it. **(1) A failed base merge reverts nothing.** The ticket merge rewinds the feature branch when its post-merge gates go red, and none of that argument survives one level up: the feature-branch gates run **before** the checkpoint, so nothing pending can call the merge bad; the base branch can have moved between the SHA we read and any later reset, so rewinding it can drop commits that were never ours; and `resetBranch`'s `git reset --hard` fallback fires when the branch to move is the one checked out, which for a *base* branch is the normal case rather than the unusual one. The only failure after a successful merge is a failed tag, and its answer is leave-it-and-escalate. **(2) A colliding tag name refuses**, before the base branch is touched. `factory/<slug>/<ISO date>` is required to be deterministic, so a second close of one feature on one day wants the same name; moving the tag would erase the marker of a delivery that already happened and suffixing it would make the determinism claim false, so the close refuses and the message distinguishes "it already points at the base tip, so this may already be closed" from "something else made this tag". **(3) The pre-approval gates run in a throwaway worktree detached at the feature branch tip**, never the main checkout — Phase 10's resolution A3 argument, unchanged, one level up. **(4) A dirty main checkout is a refusal**, because the close checks the base branch out there and the operator's uncommitted edits would otherwise end up sitting on top of a base branch they never merged into.)*

*(Added during Phase 11's review — the tag name is derived from **`paused_at`**, the instant the feature was put in front of a human, and not from the approver's clock. The approval summary quotes a tag name at checkpoint time and the close creates one at approve time; each formerly read its own clock, so an approval given the next day produced a tag whose name did not match what was approved (demonstrated with injected clocks: promised `factory/sample/2026-09-02`, created `factory/sample/2026-09-03`). One authority means the two cannot disagree, dates the delivery by when it was verified, and keeps the name stable across a retry after a failed close.)*

*(Corrected during Phase 11 — the tag is lightweight, and **being lightweight is not what makes it safe**. `src/git/git.ts` reasoned that `tag.gpgsign` "only applies to annotated tags", so `git tag <name> <ref>` needed no signing flag of its own. Probed on git 2.39.5: `tag.gpgsign=true` promotes a bare `git tag <name> <ref>` into a signed tag, which then has no message and exits 128 with `fatal: no tag message?`. A target repo that signs its tags therefore broke the feature close outright — not by hanging on a prompt, which is what the Phase 10 ledger predicted, but with an error naming a message nobody asked to write. `tag.gpgsign=false` is now in the shared `orchestratorGitConfig` alongside `commit.gpgsign=false` and `core.hooksPath=/dev/null`, and `test/integration/feature-close.test.ts` shows plain git failing in a signing repo where `ShellGit` does not.)*
