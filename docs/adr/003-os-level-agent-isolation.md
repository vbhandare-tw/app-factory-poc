...
4. **Added during Phase 5 — `GIT_CONFIG_GLOBAL=/dev/null` and `XDG_CONFIG_HOME=/dev/null` on every sandboxed child.** Layer 2 denies the home directory, which also denies `~/.gitconfig` and `~/.config/git/ignore`. Without this, every git command inside the sandbox fails `fatal: unable to access '<home>/.gitconfig': Operation not permitted` (exit 128) *before* layer 3 is ever reached — so the agent loses the read-only git it legitimately needs, and a test that only checked "git add is refused" would pass for entirely the wrong reason. Reading `~/.gitconfig` was rejected as the fix: it can carry credential helpers, signing key paths and `url.insteadOf` token rewrites. Agents never commit, so they need no identity from it.
...
One correction to the Context above, found in Phase 5 by removing our own `denyWrite` list and re-probing. On v2.1.220 the CLI's *default* sandbox already refuses `.git/hooks` and `.git/config`; what was still wide open was `.git/refs` and `.git/objects`, and `git add` succeeded. So the explicit fence is what closes the ref-moving and staging routes today, and the hook route happens to be closed twice over. That default is undocumented, unversioned and covers two of the four paths — it is not a reason to drop layer 3.
