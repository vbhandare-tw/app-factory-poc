...
const FAILURE_SCENARIOS = [
  { name: 'the CLI reports is_error', stubMode: 'api_error', expected: 'api_error' as const, validate: false },
  { name: 'the stream is truncated', stubMode: 'truncated', expected: 'crash' as const, validate: false },
  { name: 'the payload fails validation', stubMode: 'schema_violation', expected: 'schema' as const, validate: true },
];
...
describe('MockRunner and ClaudeCodeRunner agree on the same stimulus', () => {
  ...
  for (const scenario of FAILURE_SCENARIOS) {
    it(`${scenario.name} → both report the same failure`, async () => {
      ...
      const mock = await runMock(shape, mockFailure(scenario.expected));
